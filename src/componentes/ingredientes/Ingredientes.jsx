                        ...
                        <div>
                            ...
                            {pedido.map((ped)=>(
                                <div className="d-grid gap-2 col-6 mx-auto" key={ped.id}>
                                    <button className="btn btn-outline-warning d-flex justify-content-between" type="button"><img className='heigth' src={require(`../../assets/img/ingredientes/${ped.imagen}.png`)}></img>{ped.nombre} ${ped.precio} <i className="bi bi-trash-fill" onClick={()=> remove(ped.id, ped.precio)}></i></button>
                                </div>
                            ))}
                        </div>
                    }
